...
describe('resource otpvalidation', () => {
  ...
  test.skip('sendCode: only required params', async () => {
    const responsePromise = client.otpvalidation.sendCode({ message: '{{code}} is your short code to authenticate with Cool App via SMS', phoneNumber: '+346661113334' });
  ...
  test.skip('validateCode: only required params', async () => {
    const responsePromise = client.otpvalidation.validateCode({ authenticationId: 'ea0840f3-3663-4149-bd10-c7c6b8912105', code: 'AJY3' });
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
    const response = await responsePromise;
    expect(response).not.toBeInstanceOf(Response);
