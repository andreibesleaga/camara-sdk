...
describe('resource sessions', () => {
  ...
  test.skip('retrieve: request options and params are passed correctly', async () => {
    ...
    await expect(client.webrtc.sessions.retrieve('mediaSessionId', { 'x-correlator': 'b4333c46-49c0-4f62-80d7-f0ef930f1c46' }, { path: '/_stainless_unknown_path' }))
      .rejects
  ...
  test.skip('delete: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.webrtc.sessions.delete('mediaSessionId', { 'x-correlator': 'b4333c46-49c0-4f62-80d7-f0ef930f1c46' }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(Camara.NotFoundError);
  });
  ...
  test.skip('updateStatus: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.webrtc.sessions.updateStatus('mediaSessionId', {
    answer: { sdp: 'sdp' },
    callType: 'REGULAR',
    locationDetails: {
    confidence: { pdf: 'normal', value: 0 },
