...
export const handler = async (client: Camara, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(
      await maybeFilter(jq_filter, await client.callforwardingsignal.checkActiveForwardings(body)),
    );
  } catch (error) {
